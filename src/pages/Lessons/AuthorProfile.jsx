...
const AuthorProfile = () => {
    ...
    // Derive author info from the first lesson if available
    // We assume the backend validates the ID, so if we get lessons, we have the creator.
    // If no lessons, we might not have author info unless we fetch user specifically.
    // Using a fallback if no lessons found is tricky, but acceptable for now.
    // Reliable Author Extraction Logic
    // Based on the provided data structure:
    // 1. `creator` object might have `name`, `photoURL`, `email`.
    // 2. OR top-level `creatorName`, `creatorPhoto`, `creatorEmail` fields exist.
    // 3. OR `createdBy` object exists.
    let author = null;
    if (lessons.length > 0) {
        ...
        else if (firstLesson.createdBy && typeof firstLesson.createdBy === 'object') {
        ...
        return (
            <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
                <div className="text-center p-8 bg-white rounded-2xl shadow-md">
                    <UserAvatar size="xl" />
                    <h2 className="text-2xl font-bold text-gray-900 mt-4">Author info not available</h2>
                    <p className="text-gray-600 mt-2">This author hasn't posted any public lessons yet or simply doesn't exist.</p>
                </div>
            </div>
        );
    }
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="bg-white rounded-2xl shadow-lg p-8 mb-12 flex flex-col md:flex-row items-center md:items-start gap-8">
                    <div className="flex-shrink-0">
                        <UserAvatar user={author} size="xl" className="w-32 h-32 text-4xl" />
                    </div>
                    <div className="flex-1 text-center md:text-left">
                        ...
                        <div className="flex flex-wrap items-center justify-center md:justify-start gap-6 text-sm text-gray-600">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="font-semibold text-gray-900">{lessons.length}</span> Lessons Shared
                            </div>
                ...
                {/* Lessons Grid */}
                <h2 className="text-2xl font-bold text-gray-900 mb-6">Shared Lessons</h2>
                {lessons.length > 0 ? (
